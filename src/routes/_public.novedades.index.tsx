  ...
  return (
    <section className="section-padded">
      <div className="container-makrana">
        ...
        <p className="mt-3 text-muted-foreground max-w-prose">Eventos, ferias, talleres y nuevas colecciones de Makrana Home Art.</p>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
          {news.map((n) => (
            <Link key={n.id} to="/novedades/$slug" params={{ slug: n.slug }} className="makrana-card group">
              ...
              <div className="p-5">
                ...
                <p className="mt-3 text-xs text-muted-foreground">{n.published_at && new Date(n.published_at).toLocaleDateString("es-PE", { year:"numeric", month:"long", day:"numeric" })}</p>
              </div>
            </Link>
          ))}
        </div>
        {news.length === 0 && <p className="mt-16 text-center text-muted-foreground">Aún no hay novedades publicadas.</p>}
      </div>
    </section>
  );
}
