          ...
          <TableBody>
            ...
            {(d?.sales ?? []).map((s: any) => (
              <TableRow key={s.id}>
                ...
                <TableCell>
                  <Badge variant={s.status === "confirmada" ? "default" : "outline"}>
                    {s.status}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Badge variant="outline">{s.delivery_status}</Badge>
                </TableCell>
                <TableCell className="text-right tabular-nums">{moneyPEN(s.total)}</TableCell>
                <TableCell>
                  {s.receipt?.[0] ? (
                    <Link to="/cliente/comprobantes" className="text-accent underline text-xs">
                      {s.receipt[0].number}
                    </Link>
                  ) : (
                    "—"
