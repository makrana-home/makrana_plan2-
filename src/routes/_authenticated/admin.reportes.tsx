...
function ReportsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Reportes"
        description="Vista rápida del negocio: ventas, productos top, stock bajo y caja del mes."
      />
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPI
          icon={DollarSign}
          label="Ventas hoy"
          value={moneyPEN(r.sales.today)}
          sub={`${r.sales.todayCount} venta(s) confirmadas`}
        />
        <KPI
          icon={TrendingUp}
          label="Ventas del mes"
          value={moneyPEN(r.sales.month)}
          sub={`${r.sales.monthCount} venta(s) confirmadas`}
        />
        <KPI
          icon={AlertTriangle}
          label="Productos con stock bajo"
          value={String(r.lowStock.length)}
          sub="ítems por debajo del mínimo"
        />
        <KPI
          icon={Users}
          label="Clientes / leads"
          value={`${r.counts.customers} / ${r.counts.leads}`}
          sub="registrados"
        />
      </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-display flex items-center gap-2">
              <Award className="h-5 w-5 text-accent" /> Top 10 productos
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                ...
                {r.topProducts.map((p: any) => (
                  <TableRow key={p.name}>
                    ...
                    <TableCell className="text-right tabular-nums">{p.qty.toFixed(2)}</TableCell>
                ...
                {r.lowStock.map((s: any) => (
                  <TableRow key={s.product.id + s.warehouse.code}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {Number(s.product.min_stock).toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
