...
function ReceiptsPage() {
  ...
  useEffect(() => {
    list().then(setRows); /* eslint-disable-line */
  }, []);
  ...
  return (
    <div>
      <PageHeader
        title="Comprobantes"
        description="Notas de venta internas con numeración correlativa (no fiscal). Se generan al confirmar una venta."
      />
  ...
  return (
    <article className="bg-warm-white text-foreground p-6 font-sans" id="receipt-print">
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <tfoot className="font-medium">
          <tr>
            ...
            <td className="text-right tabular-nums">{moneyPEN(s.subtotal)}</td>
          </tr>
          <tr>
            <td colSpan={3} className="text-right py-1">
              Descuento
            </td>
            <td className="text-right tabular-nums">− {moneyPEN(s.discount)}</td>
          </tr>
          <tr className="border-t border-sand text-lg">
            <td colSpan={3} className="text-right py-2 font-display">
              TOTAL
            </td>
            <td className="text-right tabular-nums font-display">{moneyPEN(s.total)}</td>
          </tr>
        </tfoot>
      </table>
        ...
        <section className="mt-4">
          ...
          <ul className="text-sm">
            {s.payments.map((p: any) => (
              <li key={p.id}>
                {p.method.toUpperCase()} · {moneyPEN(p.amount)}{" "}
                {p.operation_code && `· op. ${p.operation_code}`}
              </li>
            ))}
          </ul>
        </section>
      ...
      <footer className="mt-8 pt-4 border-t border-sand text-xs text-muted-foreground text-center">
        Gracias por apoyar el arte hecho a mano. Este documento es un comprobante interno no fiscal
        de Makrana Home Art.
      </footer>
