            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                  Activa o desactiva cada bloque. Los cambios se reflejan al recargar la web
                  pública.
                </p>
              </div>
              <Button
...
function ComplaintBookAdmin() {
  ...
  const load = () => {
    ...
    listEntries()
      .then(setEntries)
      .catch((error: any) =>
        toast.error(error.message ?? "No se pudieron cargar las reclamaciones."),
      )
      .finally(() => setLoading(false));
  };
  ...
  async function saveEntry(entry: any) {
    ...
    try {
      await updateEntry({
        data: { id: entry.id, status: entry.status, admin_notes: entry.admin_notes ?? "" },
      });
    ...
    <section className="rounded-3xl border border-sand/60 bg-warm-white p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="font-display text-xl">Libro de Reclamaciones</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Solicitudes enviadas desde el formulario público.
          </p>
        </div>
        <Button
          type="button"
      ...
      {loading ? (
        <p className="mt-8 text-sm text-muted-foreground">Cargando registros...</p>
      ) : entries.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-sand p-8 text-center text-sm text-muted-foreground">
          Todavía no se han registrado reclamaciones.
        </div>
      ) : (
        <div className="mt-6 space-y-3">
          {entries.map((entry) => (
            <details
              key={entry.id}
              className="group rounded-2xl border border-sand/70 bg-cream/25 open:bg-cream/45"
            >
              <summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  <div className="flex items-center gap-2">
                    <strong className="text-sm">{entry.claim_number}</strong>
                    <StatusBadge status={entry.status} />
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {entry.first_name} {entry.first_surname} {entry.second_surname} ·{" "}
              ...
              <div className="border-t border-sand/70 p-4 sm:p-5">
                <div className="grid gap-4 text-xs sm:grid-cols-2 lg:grid-cols-3">
                  <Data
                    label="Documento"
                    value={`${entry.document_type} ${entry.document_number}`}
                  />
                  <Data label="Contacto" value={`${entry.phone} · ${entry.email}`} />
                  <Data
                    label="Ubicación"
                    value={`${entry.department}, ${entry.province}, ${entry.district}`}
                  />
                  <Data
                    label="Dirección"
                    value={`${entry.address}${entry.reference ? ` · Ref.: ${entry.reference}` : ""}`}
                  />
                  <Data label="Tipo" value={`${entry.claim_type} · ${entry.consumption_type}`} />
                  <Data label="Pedido" value={entry.order_number} />
                  <Data label="Fecha del reclamo" value={entry.claim_date} />
                  <Data label="Proveedor" value={entry.provider} />
                  <Data
                    label="Monto reclamado"
                    value={
                      entry.claimed_amount == null
                        ? "—"
                        : `S/ ${Number(entry.claimed_amount).toFixed(2)}`
                    }
                  />
                  <Data
                  ...
                  <Data
                    label="Fechas"
                    value={`Compra: ${entry.purchase_date || "—"} · Consumo: ${entry.consumption_date || "—"} · Caducidad: ${entry.expiration_date || "—"}`}
                    wide
                  />
                  <Data label="Detalle de la reclamación / queja" value={entry.claim_detail} wide />
                  <Data label="Pedido del cliente" value={entry.customer_request} wide />
                </div>
                <div className="mt-5 grid gap-4 border-t border-sand/70 pt-5 md:grid-cols-[14rem_1fr_auto] md:items-end">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEntries((current) =>
                          current.map((item) =>
                            item.id === entry.id ? { ...item, status: e.target.value } : item,
                          ),
                        )
                      }
                      className="mt-2 h-10 w-full rounded-md border border-input bg-warm-white px-3 text-sm"
                    >
                  ...
                  <div>
                    <Label htmlFor={`notes-${entry.id}`}>Notas administrativas</Label>
                    <Textarea
                      id={`notes-${entry.id}`}
                      ...
                      onChange={(e) =>
                        setEntries((current) =>
                          current.map((item) =>
                            item.id === entry.id ? { ...item, admin_notes: e.target.value } : item,
                          ),
                        )
                      }
...
function Data({ label, value, wide }: { label: string; value?: string | null; wide?: boolean }) {
  return (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${attended ? "bg-emerald-100 text-emerald-700" : status === "en_proceso" ? "bg-amber-100 text-amber-700" : "bg-sand text-foreground/70"}`}
    >
      {attended ? <CheckCircle2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
      {status.replace("_", " ")}
    </span>
  );
}
