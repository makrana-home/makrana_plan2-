...
ALTER SEQUENCE public.receipt_number_seq
  AS bigint
  MINVALUE 0;

UPDATE public.receipts
SET number = 'TMP-' || id::text;

WITH numbered_receipts AS (
  SELECT id, row_number() OVER (ORDER BY issued_at, id) - 1 AS number
  FROM public.receipts
)
UPDATE public.receipts AS receipt
SET number = 'MKR-' || lpad(numbered_receipts.number::text, 8, '0')
...
SELECT setval(
  ...
  COALESCE((
    SELECT max((regexp_replace(number, '^MKR-', ''))::bigint)
    FROM public.receipts
    WHERE number ~ '^MKR-[0-9]{8}$'
  ), 0),
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.sale_items AS si WHERE si.sale_id = _sale_id
  ) THEN
    RAISE EXCEPTION 'la venta no tiene items';
  END IF;

  FOR _item IN
    SELECT si.* FROM public.sale_items AS si WHERE si.sale_id = _sale_id
  LOOP
  ...
  _num := 'MKR-' || lpad(_next_num::text, 8, '0');

  INSERT INTO public.receipts(sale_id, number, created_by)
  VALUES (_sale_id, _num, _uid)
